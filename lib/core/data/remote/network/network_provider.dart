...
class NetworkProvider {
  ...
  Future call(
    ...
    switch (method) {
      case Method.GET:
        try {
          responseData = await client.get(url, headers: headers).timeout(timeout);
      ...
      case Method.POST:
        try {
          responseData = await client.post(url, body: json.encode(body), headers: headers).timeout(timeout);
        } on Exception catch (e) {
          if (e is TimeoutException) {
            log.i('TimeOut responseData ::: $e');
            log.i('TimeOut responseData ::: ${responseData}');
      ...
      case Method.PUT:
        try {
          responseData = await client.put(url, body: json.encode(body), headers: headers).timeout(timeout);
        } on Exception catch (e) {
          if (e is TimeoutException) {
            log.i('TimeOut responseData ::: $e');
            log.i('TimeOut responseData ::: ${responseData}');
      ...
      case Method.DELETE:
        try {
          responseData = await client.delete(url, headers: headers).timeout(timeout);
        } on Exception catch (e) {
          if (e is TimeoutException) {
            log.i('TimeOut responseData ::: $e');
            log.i('TimeOut responseData ::: ${responseData}');
